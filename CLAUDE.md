...
**Foot IK — RE-ENABLED 2026-07-05 at the AnimX-kit chain spec (AnimX controller pass, Step 1; awaiting PIE feel check).** `bEnableFootIK` C++ header default is **`true`**. The 2026-06-22 landing warp that got foot IK disabled was root-caused *then* as "engine Leg IK is unfit for the digitigrade legs" — VibeUE inspection of the kit's `AnimBP_Cat` IK layer (2026-07-05) **refined that**: the kit uses the *same* engine Leg IK warp-free; our warp came from the **chain definition** — we solved the front leg *through the paw joint* (IK goal `VB Hand`, NumBonesInLimb=4) and the back leg one bone too deep (n=4 past the Thigh). Kit shipping spec, now replicated in `ABP_Cat_V2`: **front = `VB Pastern` goal, FK `Pastern`, n=3** (Forearm/UpperArm/Shoulder; the Hand paw stays FK and rides the solved pastern), **back = `VB Foot` goal, n=3** (Hook/Shin/Thigh), MaxIterations 15, MinRotationAngle 3–5°. `UpdateFootIK` (C++) is unchanged: per-paw vertical offsets measured at the paw, fed to additive ModifyBones on the goal VBs; upward-only conform + swing-phase fade + over-extension guard + speed gate (150) + landing-frame alpha snap. **Gotcha (still true):** toggle `bEnableFootIK` via the **C++ header default**, not a BP override (inherited-default-doesn't-propagate). Full kit-math inventory + deferred milestone 1b items (paw rotation from surface normal, pelvis/chest drop + Spline IK, speed-taper alpha) in `Saved/.Aura/plans/quadruped-ik-port.md`.
