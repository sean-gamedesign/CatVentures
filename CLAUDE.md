...
**Multiplayer pass deltas (2026-07-06, commit `a1f3a70` — first 2-player test of all of the above):** (1) **turn-in-place body yaw never replicated** (orient-to-movement servers derive rotation from acceleration; a client's `SetActorRotation` at zero accel never leaves its machine) — the yaw now rides the turn RPCs (`Server_SetTurnActive/SetTurnRate` carry `BodyYaw`; throttle = rate Δ 0.05 OR 5° drift) and the server copy pursues it with **exponential** smoothing (`ClientTurnTargetYaw`, speed 12 — a fixed-rate pursuit stalls between RPC steps and jitters); (2) **body pitch = min-magnitude of TWO estimators** (capsule ±30 probes AND paw-floor differential, signs must agree) — probes alone false-read ~19° at the ramp base lip, paw-floors alone feed back through the pose to ~2× the slope; **fractional at idle** (35%, legs compensate — full-slope pitch face-plants the idle head) and full when moving; (3) **chest CP rise-only** (downhill drop bowed the front end); (4) **foot IK off during turn-in-place footwork** (`SpeedType == Turn`); (5) both cosmetic **springs integrate on a clamped step** (≤1/30 s) **with rail anti-windup** — a frame hitch made damping×dt > 1, exploding the accel-lean spring onto its ±1.5 rail with stored velocity = a PERMANENT forward bow ("stuck landing pose") on both machines; (6) **`AccelLeanStiffness` is overridden to 0 on PrimeCatBase** (C++ default 120) — a residual landing-bow report post-fix was never re-verified; re-enable + verify is the FIRST task of the next controller session (see `Saved/.Aura/plans/weighty-movement-opus48-handoff.md` §6). Session gotchas: Live Coding reported success while old code kept running (verification rounds = editor-closed rebuilds ONLY); the editor-quit-via-Python TaskGraph assertion is harmless quit-path noise.
