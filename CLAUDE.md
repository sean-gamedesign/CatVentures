...
- **The exact ABP consumption surface** (ABP_Cat_V2 polls these every update — do not rename without updating the ABP): `Speed`, `SpeedType`, `JumpPhase`, `NormalizedFallSpeed`, `AimYawInterp`, `AimPitchInterp`, `AlphaAimInterp`, `AlphaPlayBreathInterp`, `PlayRateInterp`, `TurnRateAnim`, `bGoTurn`, `LeanAmount`, `AimBSYawTime`, `AimBSPitch`, `bPlayBlink`, `bPlayEarsTwitch`, `IndexEars` (last five added with the additive layer, 2026-07-06); plus the `OnMeow` delegate. (`PlayRate` is a reserved source that is currently never written — `PlayRateInterp` always reads 0.)
...
**OPEN / tracked for the next jump pass:** **"up doesn't match the anim"** — the variable-height hold keeps launch `Vz` ~constant for ~0.18 s (rises ~123 cm of the ~125 cm jump near-instantly), so no fixed-rate rise clip matches that rocket; consider play-rate scaling the launch/apex clips. Camera juice and any physics-arc/air-control change are NOT done.
