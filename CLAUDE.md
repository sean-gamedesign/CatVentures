...
**Jump anim — movement-aware set (Step 1, 2026-06-20, jump-feel pass).** Each Jump phase state in `ABP_Cat_V2` (`Jump_Launch/Apex/Fall/Land`) is a Blend-Poses-by-Bool between the **InPlace** variant (false) and the **Run** variant (true), driven by `Speed > 200` — so a standing hop and a running leap use coherent clip sets (and the matching `Land_stop`/`Land_run`). The fall blendspaces (`BS1_Cat_Fall_inPlace/run`) were originally authored `Fall_low @0`, apex-pose `@1`, fed **`1 - NormalizedFallSpeed`** (a Subtract node in `Jump_Fall`) — apex pose at the top, easing to `Fall_low` as it accelerates. (**Sample layout remapped 2026-06-22** to apex@0 / `Fall_low`@0.5 / apex@1 so the spread `Fall_low` shows *mid-fall* instead of at impact; the `1 - NormalizedFallSpeed` feed is unchanged. This is a secondary cosmetic tweak whose persistence is unverified — UE only rebuilds a blendspace's eval grid via the editor UI — so confirm it after a reload; see *OPEN* item (1).) `NormalizedFallSpeed` is computed **continuously while descending** in `UpdateJumpPhase` (not only in the Fall case) so the blendspace axis is correct the instant Fall begins. The **`Jump_Apex` state mirrors the `Jump_Fall` blendspace** (same `1 - NormalizedFallSpeed` feed into `BS1_Cat_Fall_inPlace/run`) instead of a standalone apex clip — so Apex→Fall is a crossfade between identical poses, killing the post-apex snap (2026-06-20).
...
**Foot IK — DISABLED 2026-06-22 (engine Leg IK is unfit for the digitigrade legs; isolation-confirmed).** `bEnableFootIK` C++ header default is **`false`**. The foot-IK chain ends in 2× engine **Leg IK** (NumBonesInLimb=4), and the engine Leg IK **cannot solve the cat's 4-bone digitigrade legs**: at `FootIKAlpha=1` it **WARPS** the limbs (the long-hunted landing "stretch/deform" bug) **and** fails to actually plant the paws (penetration persisted with it ON). A per-frame landing isolation — forcing `FootIKAlpha=0` — removed the warp **entirely**, decisively proving the engine Leg IK was the cause, *after* per-frame logging had already ruled out the per-paw offsets, the Spine_2 lean, the inertialization node, and the fall blendspace. `bEnableFootIK=false` keeps `FootIKAlpha` at 0 so both Leg IK nodes pass through (no solve). Baked OFF in the base DLL 2026-06-22; `bFootIKDebugDraw` is **`false`**. **Gotcha (still true):** toggle `bEnableFootIK` via the **C++ header default**, not a BP override (inherited-default-doesn't-propagate). A real landing-penetration fix needs a **proper quadruped IK** (the kit's VB Pastern/Hook ModifyBones + Spline IK), NOT engine Leg IK — tracked, not done. (`UpdateFootIK`'s upward-only conform + speed gate + over-extension guard remain in the code, inert while disabled, for whoever builds the real IK.)

**OPEN / tracked for the next jump pass:** (1) **Landing-pose polish — the big "stretch/warp" is FIXED.** The landing stretch was the engine **Leg IK** warping the digitigrade legs — RESOLVED by disabling foot IK (see above). Residual items are milder, both confirmed by per-frame landing logging (2026-06-22) and both needing **animation/IK work, NOT parameter tweaks** (every parameter was tried):
  - **`Land_stop`→Idle snap** — on the standstill landing the body silhouette (Hand_L↔Foot_L spread) overshoots to **~58** (exceeding *every* clip's max: Land_stop 42, Fall_low 58, idle 22) then snaps back. Ruled OUT by isolation: the Spine_2 lean (logged 0 at the snap), the `Land_stop` clip (its own max is 42), **inertialization** (turning the whole jump cluster to Standard Blend changed nothing), and the **breathing additive** (forcing its alpha to 0 changed nothing). Conclusion: it's a **rotation-blend artifact** — blending the Land-recovery pose to the Idle stance interpolates the joint rotations through a *wider* configuration than either pose (classic candy-wrapper), because `Land_stop`'s recovery pose doesn't match the idle stance. Fix = **matched land/idle clips or foot-lock**, not blend-duration/logic tweaks.
  - **Paw penetration** — measured **front paws ~12 cm below ground** during Land (back paws 0). Nothing plants them with IK off — needs a real quadruped IK (Pastern/Hook), the same `#2` build. Two side-changes made while hunting the warp, to verify/keep: (a) the three **Jump→Land** transitions were set to **Standard Blend** (0.04/0.05 crossfades) — harmless, kept; (b) the **fall blendspace was remapped** (apex @0, `Fall_low` @0.5, apex @1 on `BS1_Cat_Fall_inPlace/run`) so the spread `Fall_low` pose shows mid-fall instead of at impact — **but** UE only rebuilds a blendspace's eval grid through the editor UI, *not* a scripted `set_editor_property`, so confirm the remap is actually live after an editor reload (re-open + save each blendspace in its editor if it reverted to the old grid). (2) **"up doesn't match the anim"** — the variable-height hold keeps launch `Vz` ~constant for ~0.18 s (rises ~123 cm of the ~125 cm jump near-instantly), so no fixed-rate rise clip matches that rocket; consider play-rate scaling the launch/apex clips. Camera juice and any physics-arc/air-control change are NOT done.
